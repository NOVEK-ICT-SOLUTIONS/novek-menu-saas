...
uploadRouter.post("/image", authMiddleware, upload.single("image"), (req: Request, res: Response) => {
  if (!req.file) {
    res.status(400).json({
      success: false,
...
uploadRouter.delete("/image", authMiddleware, async (req: Request, res: Response) => {
